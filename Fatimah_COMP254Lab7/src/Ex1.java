...
public class Ex1<K,V> extends AbstractSortedMap<K,V> {
    ...
    {
        //Exercise 01: alternative implementation of that method that does not rely on the use of recursion (rana)
        Position<Entry<K,V>> walk = p;
        while (isInternal(walk)) {
            int comp = compare(key, walk.getElement( ));
            if (comp == 0)
                return walk; //if key exists, return pos
            else if (comp < 0)
                walk = left(walk); //left search
            else
                walk = right(walk); //right search
        }
        return walk; //if no key found
    }
